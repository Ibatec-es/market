...
Estado previo: ACTION-029 completada. Sistema multi-cliente validado con `demo-client`.

## Objetivo

Escribir la guía completa para incorporar un nuevo cliente al sistema. El objetivo es que un desarrollador que no ha participado en el proyecto pueda seguirla y tener un cliente nuevo funcionando en menos de un día.

## Archivo a crear

`docs/brand-onboarding.md`

---

## Contenido del documento a crear
...
No con la implementación actual (build time). Cada despliegue sirve un cliente. Para multi-cliente en runtime, se necesita la Opción B de ACTION-029.
...
Seguir el propio documento para crear un tercer cliente de prueba (ficticio):

```bash
BRAND_ID="test-onboarding"
```

Crear los archivos mínimos siguiendo la guía paso a paso. Si hay algún paso poco claro o que falla, actualizar la guía antes de dar la acción por completada.

Tiempo objetivo de la prueba: menos de 30 minutos.

## Entregable

- `docs/brand-onboarding.md` completo y verificado.

## Criterio de hecho

Un desarrollador que no ha visto el proyecto puede seguir el documento y tener un tercer cliente funcionando en menos de 4 horas. Cada paso es autoexplicativo y no requiere consultar otros archivos del proyecto.
